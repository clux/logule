...
// logule can namespaces.map with this function to get correct padding + colors
C.formatNamespace = function (n, i) {
  n = (i === 0) ? pad(n + '', cfg.stdout.pad | 0) : n;
  return C.colors.namespace(n);
};

C.timestamp = makeStamper(cfg.stdout.timestamp);
C.nesting = cfg.stdout.nesting | 0;
...
if (cfg.filestream && cfg.filestream.file) {
  ...
  C.filesuppressed = cfg.stream.allow ?
    set.difference(C.levels, cfg.filestream.allow) :
    cfg.filestream.suppress;
}

// stream
if (cfg.stream && cfg.stream.name) {
  var loc = global;
  cfg.stream.name.split('.').forEach(function (x) {
    loc = loc[x];
    if (!loc[x]) {
      throw new Error("stream must be globally available");
    }
  });
  if (!loc.writable) {
    throw new Error("Stream object at " + cfg.stream.name + " must be writable");
  }

  C.stream = loc;
  C.stream.suppressed = cfg.stream.allow ?
    set.difference(C.levels, cfg.stream.allow) :
    cfg.stream.suppress;
}
