...
// store process specific variable for logule to communicate across different copies
if (!process.logule) {
  process.logule = {};
}
...
Logger.prototype._log = function (lvl, msg) {
  var isMuted = (this.removed.indexOf(lvl) >= 0);

  // stdout transport
  if (cfg.suppressed.indexOf(lvl) < 0 && (!cfg.stdoutmutable || !isMuted)) {
    var output = this.namespaces.map(cfg.formatNamespace).slice(0, cfg.nesting);
  ...
  // stream transport
  if (cfg.stream && cfg.streamsuppressed.indexOf(lvl) < 0) {
    if (!cfg.streammutable || !isMuted) {
      process.logule[cfg.stream].write({
        time : new Date(),
        level: lvl,
  ...
  if (cfg.filestream && cfg.filesuppressed.indexOf(lvl) < 0) {
    if (!cfg.filemutable || !isMuted) {
...
Logger.prototype.get = function (fn) {
  ...
  // with multiple transports cannot return NOOP if suppressed/removed here
  // If we removed chaining: could return this[fn].bind(this)
  var sub = this.sub();
  return function () {
